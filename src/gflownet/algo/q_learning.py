...
class QLearning(GFNAlgorithm):
    def __init__(
        ...
        self.graph_sampler = GraphSampler(ctx, env, self.max_len, self.max_nodes, rng)
        # self.graph_sampler.sample_temp = 0  # Greedy policy == infinitely low temperature
        self.graph_sampler.sample_temp = 0.15
        self.gamma = 1
        self.n_step = 1
        self.mellowmax_omega = False
        self.type = "ddqn"
    ...
    def compute_batch_losses(
        self, model: nn.Module, target_model: nn.Module, batch: gd.Batch, num_bootstrap: int = 0
        ...
        if self.n_step == 1:
            ...
            # We're going to use indexing to compute the n-step return. Note that we once again cheat here
            # because we know there is only reward at the end of the trajectory.
            # First we compute the batch-index where each trajectory starts
            clens = torch.cumsum(torch.cat([torch.zeros((1,), device=dev), batch.traj_lens]), 0)
            # The we compute the batch-index of each state, which is just arange
            a = torch.arange(clens[-1], device=clens.device)
            # The target index is the trajectory-index of the state n steps in the future
            target_idx = a - clens[batch_idx] + self.n_step
            # The we need to decide whether the target is V(s_{t+n}) of R(\tau), which depends on whether
            # the trajectory-index of the target is less than the length of the trajectory
            mask = target_idx < batch.traj_lens[batch_idx]
            # merge V_s and R, the target is V(s_{t+n}) if the trajectory is still ongoing, otherwise it's R(\tau)
